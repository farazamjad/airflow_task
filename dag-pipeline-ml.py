...
args = {
    "owner": "airflow",
    "retries": 1,
    "start_date": pendulum.today("UTC").add(days=-1),  # 1 means yesterday
}
...
with DAG(
    ...
    task_train_LR_model = PythonOperator(
        task_id="ml_training_Logisitic", python_callable=ml_training_Logisitic_fn
    )

    # Task 5 -Identify the best model of the two
    task_identify_best_model = PythonOperator(
        task_id="identify_best_model", python_callable=identify_best_model_fn
    )
...
(
    ...
    >> task_extract_data
    >> task_process_data
    >> [task_train_RF_model, task_train_LR_model]
    >> task_identify_best_model
)
